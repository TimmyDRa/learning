describe('Laboratory Accountant', () =>{
    beforeEach(() =>{
      //Logs in with an authorized tenant
            cy.visit('https://test.app.plateaumed-staging.com/');
            cy.get('[data-testid="username"]', {timeout:60000}).type('labacc@yopmail.com');
  ...
  it(`should login as a laboratory accountant,
    - The appropriate user should be displayed at the header `, () => {
    cy.contains('Pricing', {timeout:60000});
    cy.get('[class=" css-u97ims"]').click();
    console.log("success");
    cy.contains('Laboratory Accountant');
     });
